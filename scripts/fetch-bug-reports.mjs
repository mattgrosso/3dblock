...
//
// Reads through the Firebase CLI, not the Admin SDK: the blockoutBugReports
// node (in the games hub's thunderstoner project — this app has no Firebase
// project of its own) is write-only under the rules, and the CLI's
// project-owner login bypasses them with no service-account key to keep out
// of git. Needs `firebase login` to be valid; says so plainly if not.

import { execFile } from 'node:child_process';
import { promisify } from 'node:util';

const run = promisify(execFile);
const PROJECT = 'thunderstoner-876f8';
const PATH = '/blockoutBugReports';
