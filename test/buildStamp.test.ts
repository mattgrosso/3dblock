...
import { formatBuildTime, buildStampText } from '../src/buildStamp'
...
const localTime = (y: number, m: number, d: number, h: number, min: number): Date =>
...
describe('buildStampText', () => {
  it('is the house format: version, then when it was built', () => {
    expect(
      buildStampText({ version: '0.1.0', buildTime: localTime(2026, 8, 22, 1, 32), now: NOW }),
    ).toBe('v0.1.0 · built Aug 22, 1:32 AM')
  })

  it('degrades instead of disappearing', () => {
    expect(buildStampText({ version: '0.1.0', buildTime: null, now: NOW })).toBe('v0.1.0')
